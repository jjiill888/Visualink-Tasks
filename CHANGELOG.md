...
- **文库**：行首「☆」悬停浮现，点击收藏；已收藏「★」常显（装饰灰再叠 45% 透明度——用户反馈初版太显眼，常显态只是个置顶记号，行悬停才恢复清晰）；收藏的浮到列表首，组内保持更新时间倒序；再点取消
- **侧栏**：每行同款「☆/★」；顶部新增「收藏」快捷分区（空则不显示，可折叠键 fav）——是镜像不是搬家，笔记同时保留在原分区；「收藏」分区头不作为拖拽落点（无归组语义），也无「＋」新建
- `POST /notes/{id}/favorite` 开关；`from=panel` 返回面板片段，否则返回文库列表片段（携带当前搜索词，`FormValue` 兼容 POST body 里的 q）
- `noteCols` 增加 `IsFav`（EXISTS 子查询按 ?1 查询者算），全部笔记查询自然带上

## 2026-07-17 云笔记 · 文档组排序（侧栏拖拽）

组顺序此前固定为「组内最新文档」的出现序，无法手工安排。现在自己的组头可拖拽（复用文档归组的 DnD 基建）：

- 拖到另一个自己的组上=插到它前面（落点组顶缘一条强调蓝插入线）；拖到分区标题行=移到末尾
- `PUT /notes/groups/{gid}/move`（`before` 参数，空=末尾；仅建组者，两个组都校验属主），事务内整表重排 `sort_order`；新建组自动排末尾
- 展示规则：自己的组按自定义顺序在前，他人的组保持「最新文档」序在后；顺序是全局的——组因可见性出现在多个分区时一起变
- 老库 `ALTER TABLE` 补 `sort_order`（全零=沿用建立先后，行为不变）

## 2026-07-17 云笔记 · 导出（.md 下载 + 打印/PDF）

此前笔记内容只进不出。两条零成本路径补齐：

- **Markdown 下载**：顶栏「导出」（有读权限即可，reader 也能带走）→ `GET /notes/{id}/export`，`text/markdown` 附件；文件名用笔记标题（RFC 5987 UTF-8 编码，老客户端回退 `note-{id}.md`），文件系统不容字符统一替换为 `-`
- **打印/存 PDF**：编辑页直接 Ctrl+P——`@media print` 隐藏顶栏/侧栏/源码栏/状态字，只留标题与正文；深色主题打印强制回浅色词元（覆盖规则进 `ne-base.css`，注意书写顺序须在 `html.ne-dark` 之后才生效）
- 删除冻结态同步隐藏「导出」按钮

## 2026-07-17 云笔记 · 历史版本 diff（终于不用肉眼对快照了）

修订页此前只能看整篇快照，两个版本之间改了什么全靠肉眼。补服务端行级 diff：

- **入口**：历史弹层每行新增「对比」（与「查看/恢复」同一文字链接语言，新标签打开）
- **对比页** `GET /notes/{id}/revisions/{rid}/diff`：与上一版本（同笔记里 id 更小的最近一条）对比；首个版本无上一版，整篇视为新增。标题下一行浅灰 meta：两版时间、保存者、＋n －n 统计；顶栏右端「查看渲染快照」直达原有修订页
- **界面与笔记系统统一**（追加）：脱 base.html 旧壳改独立 ne 页（40px 安静顶栏 + 单栏居中 920px + `ne-lib-head` 标题行），主题三态按钮与文库/编辑页同款（ne-theme 全系统共享，防闪烁预判脚本），深浅双主题截图核对通过
- **配色**：ne 语义色淡底——增 = `--ne-ok` 淡底、删 = `--ne-err` 淡底 + 划线；等宽字体；超过上下文 3 行的未改动段折叠为「⋯ N 行未改动」标记
- **算法**（`internal/notes/diff.go`，零依赖）：先掐公共前后缀再做 LCS DP；掐剩中段行数乘积超 400 万退化为整段替换（极端长文保护）；空文本视为零行（否则空版本 diff 出幽灵空行）
- 验证：改行/增行/删行/折叠标记/统计数字/首版全新增/版本不存在 404 全过

## 2026-07-17 云笔记 · 文档组重命名（就地改名，仿 VSCode）

组建错名字此前只能删了重建。侧栏组行悬停浮现铅笔按钮（CSS mask 图标，与 ＋/－ 同一符号语言）：

- 点击后组名按钮就地换成无边框输入框（预填原名并全选），回车/失焦提交、Escape 取消；同名或空名视为取消不发请求
- `PUT /notes/groups/{gid}`（仅建组者，属主校验压 SQL），成功返回整块面板片段自然收尾；失败恢复原名走全局错误 Toast
- 校验与建组一致：非空、≤50 字；实测越权 403、空名/超长 400

## 2026-07-16 云笔记 · 回收站（软删除终于能找回了）

此前删除只是打 `deleted_at` 标记，数据都在库里却没有任何找回入口——误删只能进数据库捞。本次补齐整条链路：

- **入口**：文库页搜索行右侧「回收站」安静文字按钮（faint 色，不抢视线）；删除确认文案同步改为「可在回收站找回，30 天后自动清除」
- **回收站视图**：与文库列表同一行语言——标题（muted 色不可点）+ 删除时间靠右 + 悬停浮现「恢复 / 彻底删除」（恢复悬停转强调蓝、彻底删除转警示红）；顶行「← 返回文库」+ 保留期提示浅灰小字；切回标签页的自动刷新会跳过回收站视图（不把人拽回列表）
- **数据层**：`ListTrashedNotes / RecoverNote / PurgeNote / PurgeExpiredNotes`，权属全压在 SQL 的 owner_id 条件（回收站只对创建者存在，越权恢复/清除实测 403）；彻底删除靠外键 CASCADE 带走 shares/revisions/attachments 行 + handler 移除磁盘附件目录
- **自动清理**：后台任务每 12h 清一次删除超 30 天的笔记（连附件目录）
- **⚠ FTS 成对律（实测踩坑）**：`SoftDeleteNote` 会把行摘出 notes_fts（已删的不该被搜到），因此恢复必须把行塞回索引——否则恢复后搜不到，且再次删除时 FTS `'delete'` 命令对不在索引的行直接报错（第二次删除 500，e2e 抓到）；同理硬删除前也要先塞回索引，AFTER DELETE 触发器的 `'delete'` 才有的放矢。以后任何绕过 SoftDeleteNote/RecoverNote 动 `deleted_at` 的代码都要遵守这条
- 另一坑：modernc 驱动把 DATETIME 列以 RFC3339 交出，自己按 `2006-01-02 15:04:05` 解析会静默失败漏出原始串，扫 `time.Time` 让驱动解析
- 验证：删→现身回收站→FTS 搜不到→恢复→FTS 又能搜到→再删→彻底删除→回收站与 FTS 双双为空；旧代码删的历史数据同样可清；深浅双主题截图核对
...
> 注：该修复原捆绑在看板视觉重排提交（f4d114b）中，用户裁定不要看板视觉改进、整体回退后，本修复单独重新提交。

## 2026-07-10 全系统设计语言统一 · 第一步：词元提升（笔记零视觉变化）

笔记模块定型的 ne-* 极简体系升格为全系统设计语言，IM 与内部协作系统将按此迁移。本次为地基工程：

- **设计规范固化**：完整原则（灰阶色板/图标铁律/交互准则/组件惯用法/性能要求/迁移路径）写入 `.claude/skills/vl-design-language/SKILL.md`，作为后续所有 UI 工作的约束性规范。已裁定：模块识别走单一强调蓝不配模块色；变量沿用 `--ne-` 前缀；迁移顺序 词元→协作系统换壳→IM 重做；base.html 消息中心重设计时并入 IM 并更名「通知中心」
- **词元提升**：色板变量从 `notes-editor.css` 的 `body.note-standalone` 作用域抽出到新文件 `static/css/ne-base.css`（浅色挂 `:root`、深色挂 `html.ne-dark`），成为单一事实来源。`main.go` 启动时读入、模板函数 `neBaseCSS` 内联进各页 `<head>` 的 `<style>`——零额外网络请求（跨境高 RTT 不受损），任何模块的页面挂上这段即可消费全部 `--ne-*` 变量
- **文库页主题按钮图标化**：`/notes` 右上角「自动/浅色/深色」文字按钮换成与编辑页同款三枚内联 SVG（半圆/太阳/月亮）
- 验证：改动前后 文库/编辑 × 浅色/深色 四张截图逐字节一致（词元提升零视觉变化）；主题切换 e2e 6 项全过；历史只读页（无主题脚本、html 不加 `.ne-dark`）保持浅色语义不变
...
- **文档行删除（用户追加）**：自己的文档行悬停浮现「－」（与组行「＋/－」同一符号语言，确认后软删除）。DELETE /notes/{id} 按请求来源分流：HX-Request 头（文库页）返回列表片段、fetch（侧栏）返回面板片段。注意：headless Chrome 报 (hover:none) 会让悬停按钮常显，是截图环境现象非 bug，桌面浏览器仅悬停浮现
- **新建入口移到分区头（用户反馈：底栏不顺手）**：每个分区标题行（公共/共享/私人）右端悬停浮现「＋」，点击弹「新建文档 / 新建文档组」菜单（fixed 定位贴按钮，挂在列表容器外不被刷新冲掉；Escape/点外/滚动收起）。**从哪个分区新建就落在哪个分区**：/notes/new 新增 ?vis=private/restricted（非法回落公开），私人区建的直接私有。底栏只剩「文件/大纲」
- **删除当前打开的文档 → 空白冻结态（用户反馈：不要把整页跳走）**：不再跳文库——编辑区收敛为空白等待状态：编辑器岛监听 notes-doc-deleted 事件停自动保存（复用 conflicted 闸门，含 beforeunload 兜底冲刷，杜绝向已删文档发 404 保存）、退出分屏、清空并锁定编辑器（uiEditable=false + 空 setProps 重估）；页面侧标题清空只读+「文档已删除」占位、顶栏源码/历史/权限按钮隐藏；URL replaceState 归一到 /notes/new——原地址刷新只会 404，归一后刷新直接得到全新空白文档
