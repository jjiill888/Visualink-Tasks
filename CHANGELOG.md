...
- **文档行删除（用户追加）**：自己的文档行悬停浮现「－」（与组行「＋/－」同一符号语言，确认后软删除）；删除的是当前打开的文档时回文库页。DELETE /notes/{id} 按请求来源分流：HX-Request 头（文库页）返回列表片段、fetch（侧栏）返回面板片段。注意：headless Chrome 报 (hover:none) 会让悬停按钮常显，是截图环境现象非 bug，桌面浏览器仅悬停浮现
