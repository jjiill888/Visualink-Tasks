...
Markdown 兼容 CommonMark + GFM（表格对齐、任务列表、删除线、自动链接），另支持：数学公式 `$...$` / `$$...$$`（KaTeX 自托管渲染，双击编辑源码）、Emoji 短代码（`:smiley:` → 😃）、YAML front matter、代码块多语言语法高亮（highlight.js 自托管按需加载，29 种常用语言，```` ```go ````/```` ```py ```` 等标注即生效）；`[TOC]` 独占一行渲染为文档目录（点击跳转、随标题实时更新，原文保留）；内联 HTML 标签原样保留显示（不渲染执行）。
...
- 删除仅创建者可操作（软删除进回收站，保留 30 天后自动清除；文库页「回收站」入口可恢复或彻底删除）
...
| `GET /notes/trash` | 回收站视图片段（仅自己删除的笔记，新删在前） |
...
| `POST /notes/{id}/favorite` | 收藏开关（按用户各记各的）；文库置顶显示，侧栏顶部「收藏」快捷分区 |
